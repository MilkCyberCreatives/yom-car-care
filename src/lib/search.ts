...
const SYN: Record<string, string[]> = {
  ...
  'air freshener': ['air-freshener', 'airfreshener', 'parfum', 'desodorisant', 'désodorisant', 'sent-bon'],
...
/** Expand a query with synonyms (EN/FR) */
function expandQuery(q: string): string[] {
  ...
  for (const t of toks) {
    ...
    // try joining two-word category names typed as single token
...
export type SearchResult = {
  item: ProductData
  score: number
  reasons: string[]   // matched tokens for UI
}
...
/** Score a single product against expanded tokens */
function scoreProduct(p: ProductData, expanded: string[], opts?: SearchOptions): SearchResult {
  ...
  for (const t of expanded) {
    ...
    for (const u of tt) {
      ...
      if (name.includes(u)) { score += 3; reasons.push(u) }
      else if (cat.includes(u)) { score += 2; reasons.push(u) }
    }
  }

  // Category boost (if provided)
...
/** Public: search across catalog with EN/FR synonyms + optional category boosts */
export function searchProducts(query: string, opts?: SearchOptions) {
  ...
  const scored = products.map(p => scoreProduct(p, expanded, opts)).filter(r => r.score > 0)
