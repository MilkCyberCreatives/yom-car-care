...
export default function SearchBar({ className = '' }: { className?: string }) {
  ...
  const { items, results } = useMemo(() => {
    if (!q.trim()) return { items: [], results: [] as ReturnType<typeof searchProducts>['results'] }
      ...
      {open && q.trim() && (
        <div className="absolute z-50 mt-2 w-full rounded-xl border border-white/10 bg-zinc-950 shadow-2xl">
          {items.length ? (
              ...
              {items.map((r, i) => (
                <li key={r.item.slug}>
                  ...
                  >
                    ...
                    <span className="ml-3 text-xs text-white/60 capitalize">{r.item.category.replace(/-/g, ' ')}</span>
                  </button>
                </li>
              ))}
            ...
            <div className="px-3 py-3 text-sm text-white/70">No suggestions. Press Enter to search.</div>
          )}
        </div>
      )}
    </div>
