...
export default function MainHeader() {
  ...
  const NAV: NavItem[] = [
    { href: '/', label: 'Home' }, // keep "Home" neutral
  ...
  const isActive = (href: string) => (href === '/' ? pathname === '/' || pathname === '/fr' : pathname.startsWith(href))
  ...
  return (
    <header className="sticky top-0 z-50 bg-[var(--brand-blue)] text-white">
      <div className="container-px flex items-center gap-4 justify-between py-3">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-3" aria-label="YOM Car Care — Home">
          <Image src="/logo.svg" alt="YOM Car Care" width={150} height={40} priority
        ...
        <div className="hidden lg:flex items-center gap-8">
          <nav className="flex items-center gap-8">
            ...
            <div className="relative group">
              ...
              <div className="invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-opacity
                              absolute left-0 mt-2 w-64 rounded-xl bg-zinc-900/95 border border-white/10 p-2 shadow-2xl">
                {CATEGORIES.map(c => (
          ...
          <a href="tel:+243848994045" className="btn-ghost"><Phone size={18}/>{t.common.call_now}</a>
          <Link href="/products" className="btn-primary">{t.common.browse}</Link>
        </div>

        {/* Mobile toggle */}
        <button className="lg:hidden p-2 rounded-md bg-white/15 hover:bg-white/25"
                onClick={() => setMobileOpen(true)} aria-label="Open menu" type="button">
          <Menu />
        </button>
      </div>
        ...
        <div className="lg:hidden fixed inset-0 z-[60]">
          ...
          <div className="absolute right-0 top-0 h-full w-[86%] max-w-sm bg-zinc-950 text-white border-l border-white/10 shadow-2xl p-4 flex flex-col">
            ...
            <nav className="mt-4 space-y-2">
              ...
              <div className="mt-2">
                <button type="button"
                  ...
                  onClick={() => setMobileCatsOpen(v=>!v)} aria-expanded={mobileCatsOpen}>
                  ...
                  <div className="mt-1 ml-2 flex flex-col">
                    {CATEGORIES.map(c=>(
                      <Link key={c.href} href={c.href} onClick={() => setMobileOpen(false)}
                        className="rounded-md px-3 py-2 text-white/80 hover:text-white hover:bg-white/5">{c.label}</Link>
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="mt-auto pt-4 space-y-2">
              <a href="tel:+243848994045" className="btn-ghost w-full justify-center"><Phone size={18}/>{t.common.call_now}</a>
              <Link href="/products" className="btn-primary w-full justify-center" onClick={() => setMobileOpen(false)}>{t.common.browse}</Link>
